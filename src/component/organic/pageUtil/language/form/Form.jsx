...
import { addOne, editOne, getOne } from '../../../../../api/endpoint/language/languageApi';
...
const Form = () => {
  ...
  const handleClickSubmit = async () => {
    const res = await addOne({ label: input }, getJWT());
    if (res.status === true) {
      setNotification({
        message: res.message,
        type: 'success',
  ...
  const handleEdit = async () => {
    setWillEdit(false);
    const res = await editOne(id, { label: input }, getJWT());
    if (res.status === true) {
      setNotification({
        message: res.message,
        type: 'success',
